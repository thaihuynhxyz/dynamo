...
main()
{
  ...
  assert(!broker.HandleRequest(Request{}).ok());
  Request missing_mode;
  missing_mode.mutable_restore()->set_source_path(source.string());
  assert(!broker.HandleRequest(missing_mode).ok());
  ...
  assert(!broker.HandleRequest(unspecified_mode).ok());
  Request missing_destination;
  missing_destination.mutable_prepare_checkpoint();
  assert(!broker.HandleRequest(missing_destination).ok());
  ...
  auto prepared = broker.HandleRequest(MakePrepare(final.string()));
  assert(prepared.ok());
  std::ofstream(fs::path(prepared.directory_path()) / "image.img") << "image";
  auto committed = broker.HandleRequest(MakeCommit(prepared.transaction_id()));
  assert(committed.ok() && fs::exists(final / "image.img"));
  assert(!broker.HandleRequest(MakePrepare(outside.string())).ok());
  auto aborted = broker.HandleRequest(MakePrepare((checkpoints / "two").string()));
  assert(aborted.ok() && broker.HandleRequest(MakeAbort(aborted.transaction_id())).ok());
  assert(!broker.HandleRequest(MakeCommit(aborted.transaction_id())).ok());
  fs::remove_all(root);
}

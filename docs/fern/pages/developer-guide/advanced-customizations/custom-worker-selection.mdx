...
| `agent_context()` | Optional agent-session metadata described below |
...
`agent_context()` returns `None` for requests without agent-session metadata. When present, it provides the complete agent context received by worker selection:
...
If per-tier overlap data is unavailable, `device_overlap_blocks()` reports the available local overlap. These accessors return raw block counts, so each custom scorer chooses how to combine them.

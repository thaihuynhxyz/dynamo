...
Dynamo also recognizes the current stable identity headers emitted by the following coding agents. See [Agent Harnesses](agent-harnesses.mdx) for packaged setup.
...
<Tabs>
  ...
  <Tab title="Codex">
    ...
    Codex's `session-id` identifies the overall run and is not used as an agent session ID.
  </Tab>
  <Tab title="OpenCode">
    Dynamo maps:
